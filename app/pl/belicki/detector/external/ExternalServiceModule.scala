...
class ExternalServiceModule extends AbstractModule {
  private lazy val config = ExternalServiceConfig(
    ...
    cacheTtl = {
      val javaDuration =
        ConfigFactory.load().getDuration("external.config.cacheTtl")
  ...
  override def configure(): Unit = {
    ...
    bind(classOf[ExternalService])
      .annotatedWith(Names.named("underlying"))
      .to(classOf[GoogleWebRiskAPI])
    bind(classOf[ExternalService])
      .annotatedWith(Names.named("usedExternalService"))
      .to(classOf[MemoizingExternalService])
  }
